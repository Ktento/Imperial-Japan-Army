...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        $result = updateComments($media_comment_id, $comment_category, $media_comment);
        if (is_array($result)) {
            $errors = $result;
            echo $errors;
        } else {
...
<body>
    <div class="p-4 mx-12 max-w-6xl min-w-80 mx-auto">
        ...
        <main class="bg-gray-100 p-4 mt-4">
            ...
            <form action="media-res-upd.php?mi=<?= htmlspecialchars($media_id) ?>&mci=<?= htmlspecialchars($media_comment_id) ?>&t=<?= htmlspecialchars($title) ?>&c=<?= htmlspecialchars($category) ?>&a=<?= htmlspecialchars($target) ?>" method="POST" class="space-y-3">
                <fieldset>
                    <dl>
                    ...
                    <dl class="py-2">
                        ...
                        <dd class="ml-64">
                            <textarea name="media_comment" class="w-80 h-40 overflow-y-scroll p-2 text-left resize-none" style="resize: none;"><?= htmlspecialchars($content) ?></textarea>
                        </dd>
                    </dl>
                    <dl class="py-2">
