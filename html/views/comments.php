        ...
        <div class="bg-gray-100 py-5 px-6 mb-1 flex relative">
            ...
            <div class="w-3/4 flex justify-between items-start pr-4">
                ...
                <div class="flex space-x-2">
                    ...
                    <?php else: ?>
                        <a class="p-1 text-xs border border-gray-300 hover:bg-gray-200" href="<?= $filename ?>-res-upd.php?tci=<?= $comment_id ?>&ti=<?= $topic_id ?>&t=<?= $title ?>&c=<?= $category ?>&a=<?= $target ?>"">編集</a>
                        <a class=" p-1 text-xs border border-red-300 text-red-600 hover:bg-red-200" href="<?= $filename ?>-res-del.php?tci=<?= $comment_id ?>&ti=<?= $topic_id ?>&t=<?= $title ?>&c=<?= $category ?>&a=<?= $target ?>" onclick="return confirm('本当に削除しますか？');">削除</a>
                    <?php endif; ?>
                </div>
            </div>
