...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        $result = insertTopic($register_num, $title, $user_id, $category, $target, $tags);
        if (is_array($result)) {
            $errors = $result;
        } else {
            $success_message = "トピックが正常に登録されました (ID: $result)";
        }
    }
}
?>
                    ...
                    <dl class="py-2">
                        ...
                        <dd class="ml-64">
                            <div class="mt-2 flex items-center gap-2">
                                <label class="">
                                    <input type="radio" name="category" value="お知らせ" class="h-4 w-4 text-gray-600 border-gray-300 accent-gray-800">
                                    <span class="ml-2 text-gray-700">お知らせ</span>
                                </label>
                                <label class="">
                                    <input type="radio" name="category" value="ニュース" class="h-4 w-4 text-gray-600 border-gray-300 accent-gray-800">
